...
function resetGameLobby(gameNaam) {
    db.collection('groepen').doc(currentGroup).collection('games').doc(gameNaam).set({ 
        fase: 'wachten', host: "", spelers: [], actief: false, klaar: false 
    });
}
...
function luisterNaarTijdbom() {
    ...
    unsubscribeBom = db.collection('groepen').doc(currentGroup).collection('tijdbom').doc('status').onSnapshot((doc) => {
        ...
        if (data.actief) {
            ...
            if(statusTekst) statusTekst.innerText = `Bom is bij ${data.houder.toUpperCase()}!`;
            
            if (data.houder === currentUser) { 
                if(modalBom) modalBom.style.backgroundColor = '#ff3b30'; 
                ...
                if(modalBom) modalBom.style.backgroundColor = '#050505'; 
...
```Dat klinkt inderdaad als een typisch "race-condition" probleem! Wat er gebeurt is dat je app de functies voor de spellen wil inladen, maar de variabelen uit `app.js` (zoals wie er is ingelogd) waren de milliseconde ervoor nóg niet beschikbaar. En als het eenmaal fout gaat op die manier, dan crasht de app in de achtergrond en reageert geen enkele host-knop meer.

Ik heb de oplossing precies gebouwd zoals je vroeg:
1. **Ik ga je nú `games.js` geven.**
2. Daarna vraag je me om `app.js` in de volgende prompt.

*Daarnaast zijn de volgende fixes meegenomen in dit bestand (`games.js`):*
* **Sjaak Stem-Bug Gefixt:** Het stemmen werkte niet omdat hij het formulier verkeerd opsloeg, hij telt ze nu feilloos op en verdeelt de strafpunten.
* **Bordspel Start:** Je kunt hem nu weer hosten zonder crash.

Hier is **`games.js`** (Let op, je hoeft je `index.html` niet meer te veranderen, die was al perfect!).

### Kopieer dit volledig in `games.js`
...
function luisterNaarQuiplash() {
    ...
    db.collection('groepen').doc(currentGroup).collection('games').doc('quiplash').onSnapshot(doc => {
        ...
        if (qlFase === 'antwoorden') {
            ...
            resLijst.innerHTML = '';
            document.getElementById('btn-ql-volgende').style.display = (data.host === currentUser) ? 'inline-block' : 'none';
...
function startQuiplashRonde() {
    let qlSpelers = [];
    document.querySelectorAll('#quiplash-spelers-lijst .lobby-speler-badge').forEach(el => qlSpelers.push(el.innerText.toLowerCase()));
            ...
            shakeTimerInterval = setInterval(() => {
                ...
                if(rest <= 0) {
                    ...
                    if(d.host === currentUser) {
                        setTimeout(() => {
                            db.collection('groepen').doc(currentGroup).collection('games').doc('shake').update({fase: 'resultaat'});
                        }, 1500);
                    }
                }
            }, 1000);
...
function luisterNaarBordspel() {
    ...
    db.collection('groepen').doc(currentGroup).collection('games').doc('bordspel').onSnapshot(doc => {
        ...
        let uiActief = document.getElementById('bordspel-actief');
        if(!uiActief) return;

        if (d.fase === 'wachten' || d.fase === 'lobby') {
            uiActief.style.display = 'none';
...
function gooiDobbelsteenBordspel() {
    ...
    db.collection('groepen').doc(currentGroup).collection('games').doc('bordspel').get().then(doc => {
        ...
        if (nieuwePos >= 39) nieuwePos = 39; 
        
        let opdrachtTekst = bordOpdrachten[Math.floor(Math.random() * bordOpdrachten.length)];
        if (nieuwePos === 39) opdrachtTekst = "🏆 JE BENT GEFINISHT! Deel 10 slokken uit en trek zelf een Atje om het te vieren!";
        
        db.collection('groepen').doc(currentGroup).collection('games').doc('bordspel').update({
            [`posities.${currentUser}`]: nieuwePos,
            dobbel_uitslag: gooi,
